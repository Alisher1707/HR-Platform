...
// Bu chegaradan past farqlar — oddiy tarmoq kechikishi/orqada qolgan
// (backlog) hodisalar uchun normal. Undan yuqorisi — qurilma soati
// siljigan bo'lishi mumkinligidan darak beradi, shuning uchun log qilinadi.
const DEVICE_CLOCK_DRIFT_WARNING_MS = 5 * 60 * 1000; // 5 daqiqa
...
export async function receiveDeviceEvent(req, res) {
  ...
  // Qurilma soati asta-sekin siljib ketishi (drift) — oqilonalik chegarasi
  // ichida qolgani uchun extractEventDateTime buni rad etmaydi, lekin bu
  // haqiqatan ham kamera soati muammosi bo'lishi mumkin. Shuning uchun har
  // safar server bilan solishtirilgan farq DEVICE_CLOCK_DRIFT_WARNING_MS'dan
  // oshsa, alohida, aniq ko'rinadigan ogohlantirish bilan log qilinadi —
  // muammoni yashirin qoldirmaslik, kimdir kamera soatini tekshirishi kerak.
  if (eventDateTime) {
    const driftMs = Math.abs(Date.now() - eventDateTime.getTime());
